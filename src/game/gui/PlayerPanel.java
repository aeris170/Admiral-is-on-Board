...
public final class PlayerPanel extends JPanel {
	...
	public void paintComponent(final Graphics g) {
		...
		for(int i = 0; i < owner.getTiles().length; i++) {
			for(int j = 0; j < owner.getTiles()[i].length; j++) {
		...
		final List<Coordinate> hitCoordinates = owner.getHitCoordinates();
		...
		final List<Coordinate> missedCoordinates = owner.getMissedCoordinates();
